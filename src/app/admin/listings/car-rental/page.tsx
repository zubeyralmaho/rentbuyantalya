...
export default function CarListingsPage() {
  ...
  const fetchListings = async () => {
    try {
      const response = await fetch('/api/admin/listings/car-rental');
  ...
  // Başlık tercihi TR ise onu döndür, yoksa marka + model
  ...
  const filtered = useMemo(() => {
    ...
    return listings.filter((l) => {
      ...
      const meta = `${l.metadata?.brand ?? ''} ${l.metadata?.model ?? ''} ${l.location ?? ''}`.toLowerCase();
  ...
  const deleteListing = async (id: string) => {
    if (!confirm('Bu araç ilanını silmek istediğinizden emin misiniz?')) {
      return;
    }
  ...
  return (
    <div className="admin-page">
      <div className="page-header">
        ...
        <Link href="/admin/listings/car-rental/form" className="add-button">+ Yeni Araç İlanı</Link>
      </div>

      {filtered.length === 0 ? (
        <div className="empty-state">
          <p>{listings.length === 0 ? 'Henüz araç ilanı bulunmamaktadır.' : 'Sonuç bulunamadı.'}</p>
          <Link href="/admin/listings/car-rental/form" className="add-button">
            İlk Araç İlanını Ekle
          </Link>
        </div>
      ) : (
            ...
            <div key={listing.id} className="listing-card">
              ...
              <div className="listing-info">
                ...
                <div className="listing-details">
                  ...
                  <p><strong>Vites:</strong> {listing.metadata?.transmission}</p>
                  <p><strong>Yakıt:</strong> {listing.metadata?.fuelType}</p>
                ...
                <div className="listing-actions">
                  <Link 
                    href={`/admin/listings/car-rental/form?id=${listing.id}`}
