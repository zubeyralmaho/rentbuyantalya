...
export default function VipTransferListingsPage() {
  ...
  const fetchListings = async () => {
    try {
      const response = await fetch('/api/admin/listings/vip-transfer');
  ...
  const filtered = useMemo(() => {
    ...
    return listings.filter((l) => {
      ...
      const meta = `${l.metadata?.vehicleType ?? ''} ${l.metadata?.brand ?? ''} ${l.metadata?.model ?? ''} ${l.location ?? ''}`.toLowerCase();
  ...
  const deleteListing = async (id: string) => {
    if (!confirm('Bu VIP transfer ilanını silmek istediğinizden emin misiniz?')) {
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        alert('VIP transfer ilanı silindi!');
  ...
  const getPriceText = (listing: Listing) => {
    ...
    switch (priceType) {
      case 'per_trip':
        return `€${listing.price_per_day} / Sefer`;
      case 'per_hour':
        return `€${listing.price_per_day} / Saat`;
      case 'per_day':
        return `€${listing.price_per_day} / Gün`;
      default:
  ...
  return (
    <div className="admin-page">
      <div className="page-header">
        ...
        <Link href="/admin/listings/vip-transfer/form" className="add-button">+ Yeni VIP Transfer İlanı</Link>
      </div>

      {filtered.length === 0 ? (
        <div className="empty-state">
          <p>{listings.length === 0 ? 'Henüz VIP transfer ilanı bulunmamaktadır.' : 'Sonuç bulunamadı.'}</p>
          <Link href="/admin/listings/vip-transfer/form" className="add-button">
            İlk VIP Transfer İlanını Ekle
          </Link>
        </div>
      ) : (
            ...
            <div key={listing.id} className="listing-card">
              ...
              <div className="listing-info">
                ...
                <div className="listing-details">
                  <p><strong>Araç Türü:</strong> {listing.metadata?.vehicleType}</p>
                  ...
                  <p><strong>Şoför:</strong> {listing.metadata?.driverIncluded ? 'Dahil' : 'Opsiyonel'}</p>
                  <p><strong>Bekleme Süresi:</strong> {listing.metadata?.waitingTime} dk</p>
                  <p><strong>Rota Sayısı:</strong> {listing.metadata?.routes?.length || 0}</p>
                ...
                <div className="listing-actions">
                  <Link 
                    href={`/admin/listings/vip-transfer/form?id=${listing.id}`}
