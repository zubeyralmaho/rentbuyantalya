...
export default function VillaListingsPage() {
  ...
  const fetchListings = async () => {
    try {
      const response = await fetch('/api/admin/listings/villa-rental');
  ...
  const getTurkishTitle = (listing: Listing) => {
    ...
    return turkishTranslation?.title || 'Başlık Yok';
  };
  ...
  const filtered = useMemo(() => {
    ...
    return listings.filter((l) => {
      ...
      const meta = `${l.location ?? ''}`.toLowerCase();
  ...
  const deleteListing = async (id: string) => {
    if (!confirm('Bu villa ilanını silmek istediğinizden emin misiniz?')) {
      return;
    }
  ...
  return (
    <div className="admin-page">
      <div className="page-header">
        ...
        <Link href="/admin/listings/villa-rental/form" className="add-button">+ Yeni Villa İlanı</Link>
      </div>

      {filtered.length === 0 ? (
        <div className="empty-state">
          <p>{listings.length === 0 ? 'Henüz villa ilanı bulunmamaktadır.' : 'Sonuç bulunamadı.'}</p>
          <Link href="/admin/listings/villa-rental/form" className="add-button">
            İlk Villa İlanını Ekle
          </Link>
        </div>
      ) : (
            ...
            <div key={listing.id} className="listing-card">
              ...
              <div className="listing-info">
                ...
                <div className="listing-actions">
                  <Link 
                    href={`/admin/listings/villa-rental/form?id=${listing.id}`}
