...
export async function GET(request: NextRequest) {
  try {
    ...
    const boatType = url.searchParams.get('boatType')
    
    const supabase = await createClient()
    
    // Build query for boat rentals
    let query = supabase
      .from('listings')
      .select(`
        *,
    ...
    if (boatType) {
      query = query.contains('metadata', { boatType })
    }
    ...
    if (error) {
      console.error('Error fetching boat rental listings:', error)
      return NextResponse.json(
        { error: 'Failed to fetch boat rental listings' },
        { status: 500 }
      )
    }
    ...
    console.error('Error in boat rental listings API:', error)
    return NextResponse.json(
