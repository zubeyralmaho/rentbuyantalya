...
export async function GET(
  ...
  try {
    ...
    // Get apart rental listing with service and i18n info
    const { data: listing, error: listingError } = await supabase
      ...
      `)
      .eq('slug', slug)
      .eq('services.slug', 'apart-rental')
      .eq('active', true)
      .single()

    if (listingError || !listing) {
      console.error('Apart rental listing not found:', listingError);
      return NextResponse.json(
        { error: 'Apart rental listing not found' },
        { status: 404 }
      )
    }
    ...
    console.error('Error in apart rental API:', error)
    return NextResponse.json(
