...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('Error fetching apart rental listings:', error)
      return NextResponse.json(
        { error: 'Failed to fetch apart rental listings' },
        { status: 500 }
      )
    }
    ...
    console.error('Error in apart rental listings API:', error)
    return NextResponse.json(
